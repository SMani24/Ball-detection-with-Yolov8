...
def main():
    ...
    for vidPath in os.listdir("./Video"): # Get all directories in "Video" folder
        ...
        while success:
            ...
            # lower = np.array([7, 88, 167], np.uint8)
            # upper = np.array([26,255,255], np.uint8)
            # ballMask = cv2.inRange(hsvFrame, lower, upper)
            ...
            # plot_bboxes(frame, result.boxes.data, score=True)
            detectedFrame = cv2.resize(detectedFrame, (1280,720))
            t2 = time.time()
            dt = t2 - t1
            print(t2, " ", t1)
            print(dt)
